...
describe("headers", () => {
    describe("main-header", () => {
        ...
        it("reads back the zip64 end of central directory record it wrote", () => {
            const source = new mainHeader();
            source.totalEntries = 0x10000;
            source.size = 123;
            source.offset = 456;

            // only the zip64 end of central directory record is handed to loadFromBinary
            const zip64Record = source.toBinary().slice(0, Constants.ZIP64HDR);

            const mainh = new mainHeader();
            mainh.loadFromBinary(zip64Record);

            expect(mainh.diskEntries).to.equal(0x10000);
            expect(mainh.totalEntries).to.equal(0x10000);
            // central directory size, not the size of the zip64 record itself
            expect(mainh.size).to.equal(123);
            expect(mainh.offset).to.equal(456);
            expect(mainh.commentLength).to.equal(0);
        });
